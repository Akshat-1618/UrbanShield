...
exports.createIncident = async (req, res) => {
  try {
    ...
    const selectedNode =
      Object.entries(cityGraph.nodes).find
      (([nodeId, node]) =>node.name === areaName);
    ...
    const [nodeId, nodeData] = selectedNode;
...
exports.autoAssignUnit = async (req, res) => {
  try {
    ...
    if (!bestUnit || !bestRoute) {
      return res.status(404).json({
        success: false,
        message: "No reachable unit found",
      });
    }
    ...
    incident.routeDistance = bestRoute.totalCost;
